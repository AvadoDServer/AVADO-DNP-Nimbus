...
def main():
    ...
    # Every file that carries the package version or the upstream version.
    # All of them must move together:
    ...
    for path in dappnode_package_paths:
        ...
        update_dappnode_package(path, new_package_version, latest_nimbus_version)

    for path in docker_compose_paths:
        print(f"Updating {path.relative_to(repo_root)}...")
        update_docker_compose(path, new_package_version, latest_nimbus_version)
